<template>
    <div>
        <h3>跨域请求</h3>
        <button @click="requestPost">以POST方法请求接口</button>
        <button @click="requestGet">以GET方法请求接口</button>
...
function requestPost() {
    loading.value = true;
    const promiseData = proxy.$protoAxios.post('http://localhost:4025/api/Training/Demo', { 'name': '1' })
    promiseData.then((res) => {
        if (res.status === 200) {
            if (res.data.StatusCode === 'Success') {
                loading.value = false
...
function reqIntercept() {
    loading.value = true
    const promiseData = proxy.$HelwayAxios.post('http://localhost:4025/api/Training/Demo', { 'name': '拦截器' });
    promiseData.then((res) => {
        loading.value = false;
        data.value = res.JsonArrayCompanies
    })
